 ...
 * showcase/scripts/sync-content.ts, so there is zero drift between what
 * is documented and what the component actually does.
 */
export async function ComponentPreview({ demo, export: exportName, align = "center" }: ComponentPreviewProps) {
    const module = demoRegistry[demo];
    if (!module) {
        throw new Error(`[ComponentPreview] unknown demo id "${demo}". Run \`pnpm run showcase:sync\` and check components/**/*.demo.tsx.`);
    }
            ...
            <Tab value="Code">
                {highlighted ?? (
                    <p className="text-sm text-tertiary">
                        Source unavailable -- run <code>pnpm run showcase:sync</code>.
                    </p>
                )}
            </Tab>
        </Tabs>
