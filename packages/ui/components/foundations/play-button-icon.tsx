...
export const PlayButtonIcon = ({ isPlaying = false, ...props }: HTMLAttributes<HTMLDivElement> & { isPlaying?: boolean }) => {
    return (
        <div
            {...props}
            className={cx(
                "flex size-20 items-center justify-center rounded-full bg-white/30 backdrop-blur transition duration-100 ease-linear group-hover:bg-white/50 hover:bg-white/50",
                props.className,
            )}
        >
