    ...
    return (
        <main className="mx-auto flex max-w-container flex-1 flex-col items-center px-4 py-20 text-center md:py-28">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                {/* Button renders a plain <a>, not next/link, so the base path
                    must be prefixed by hand for the GitHub Pages deployment --
                    and the target is the concrete intro page because the /docs
                    redirect only exists on server deployments. */}
                <Button size="lg" href={`${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/docs/getting-started/introduction`} iconTrailing={ArrowRight}>
                    Browse components
                </Button>
                <Button size="lg" color="secondary" href={`${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/docs/getting-started/introduction`}>
                    Getting started
                </Button>
            </div>
